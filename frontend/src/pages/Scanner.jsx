...
const Scanner = () => {
    ...
    const [phase1Time, setPhase1Time] = useState(null);
    ...
    const handleViewProductDetails = async (productUrl, platform, name) => {
        if (!productUrl) {
        ...
        const cleanup = evaluateProductStream(url, {
            ...
            onProductEvaluating: (index, _name, step) => {
                setProducts(prev => prev.map(p =>
                    p.index === index ? { ...p, status: step, stepMessage: `Starting ${step}...` } : p
                ));
            },
            onProductStep: (index, step, message) => {
                setProducts(prev => prev.map(p =>
                    p.index === index ? { ...p, status: step, stepMessage: message } : p
                ));
            },
                                    ...
                                    <div style={{
                                        ...
                                        {selectedProduct.ai_analysis.recommendations && selectedProduct.ai_analysis.recommendations.length > 0 && (
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                                ...
                                                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                                    {selectedProduct.ai_analysis.recommendations.map((rec, i) => (
                                                        <motion.div
                                                            key={i}
